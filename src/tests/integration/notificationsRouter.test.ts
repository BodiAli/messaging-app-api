...
import type { Friendship, Notification, User } from "../../generated/prisma/index.js";
...
interface NotificationsResponse {
  notifications: (Omit<Notification, "groupChatId" | "friendRequestId" | "userId"> & {
    friendRequest: Omit<Friendship, "senderId"> & {
      sender: Omit<User, "password" | "lastSeen" | "isGuest">;
    };
  })[];
}
  ...
  describe("get logged in user's notifications GET /notifications/me", () => {
    describe("given sent friend requests to userA", () => {
      ...
      it("should return 200 status with userA's notifications", async () => {
        ...
        const typedResponseBody = response.body as NotificationsResponse;

        expect(typedResponseBody.notifications).toStrictEqual([
          {
